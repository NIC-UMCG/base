...
usage()
{
    ...
    dcm2niix_version=`dcm2niix | grep version`

    echo
    echo -e "\tVersions:"
    echo -e "\t\t*) dcm2niix : $dcm2niix_version"
...
copy_data()
{
    ...
    ## no copy required: cpa -R --progress $INPUTDIR /tmp/	
...
convert_to_nii() {
    ...
    IMAFILECOUNT=`find $INPUTDIR | grep ".dcm$" | wc -l`
    IMAFILES=`find $INPUTDIR | grep ".dcm$"`
